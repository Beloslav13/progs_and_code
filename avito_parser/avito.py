...
base_url = 'https://www.avito.ru/belgorod/avtomobili/peugeot?cd=1&radius=200'
...
def avito_pars(url, headers, requests=requests):
    ...
    if requests.status_code == 200:
        ...
        try:
            ...
            count = int(pagination[-2].text) + 1
            for i in range(1, count + 1):
                url = f'https://www.avito.ru/belgorod/avtomobili/peugeot?p={i}&cd=1&radius=200'
                if url not in urls:
                    urls.append(url)
        except:
            pass
